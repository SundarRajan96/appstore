...
class AppStore extends Component {
  ...
  getFilterApps = searchApp => {
    const {activeTabId} = this.state
    const filterApps = searchApp.filter(
  ...
  getSearchResult = () => {
    ...
    const searchResult = appsList.filter(searchDetails =>
      searchDetails.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return searchResult
  }
  ...
  render() {
    ...
    const search = this.getSearchResult()
    const app = this.getFilterApps(search)
